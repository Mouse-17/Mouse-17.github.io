<script setup lang="ts">
    ...
    const fetchYard = async () => {
        try {
            ...
            if (data.status === 'success') {
                ...
                // Lấy đánh giá ngay sau khi nhận được thông tin sân
    ...
    const fetchYardRatings = async (yardId) => {
        try {
            // Sử dụng endpoint chính xác để lấy đánh giá của sân
            // Route in api.php is: Route::get('/san/{id}/danh-gia', [SanController::class, 'getSanRatings']);
            const response = await fetch(`http://localhost:8000/api/san/${yardId}/danh-gia`);
            ...
            console.log('API đánh giá sân trả về:', data);
            ...
            if (data.status === 'success') {
                ...
                if (Array.isArray(data.data) && data.data.length > 0) {
                    // Gán trực tiếp vào sân
                    if (yard_store.value) {
                        yard_store.value.danh_gia = data.data;
                    }
                ...
                console.warn('API đánh giá sân trả về không thành công:', data);
            }
        } catch (error) {
            console.error('Lỗi khi tải đánh giá sân:', error);
            // Đảm bảo UI vẫn hiển thị đúng mặc dù có lỗi
    ...
    const fetchTimeSlots = async () => {
        try {
            const response = await fetch('http://localhost:8000/api/khung-gio');
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            
            const data = await response.json();
            console.log('API khung giờ trả về:', data);
    ...
    const fetchPopularYards = async () => {
        try {
            // Sửa endpoint thành '/api/san-pho-bien' thay vì '/api/san/phobien'
...
<template>
    <main v-if="yard_store">
                    ...
                    <div class="col-12 col-lg-6 p-0">
                        <div class="px-lg-2 px-3">
                            <div class="main-image-container">
                                <img :src="getImageSrc(yard_store.Hinh_anh)" :alt="yard_store.Ten_san" class="rounded-4">
                            </div>
                            <div class="d-flex align-items-center gap-2 mt-2 overflow-hidden" style="width: 100%;">
                                <div class="custom-thumnail__box">
                                    <img :src="getImageSrc(yard_store.Hinh_anh)" :alt="yard_store.Ten_san" class="img-fluid">
                                </div>
                                <div class="custom-thumnail__box">
                                    <img :src="getImageSrc(yard_store.Hinh_anh)" :alt="yard_store.Ten_san" class="img-fluid">
                                </div>
                    ...
                    <div class="col-12 col-lg-6 p-0">
                        <div class="padding-custom ps-lg-5">
                            ...
                            <div class="d-flex align-items-center gap-2 mt-2 mb-4">
                                ...
                                <a href="#" class="m-0 fs-4 fw-regular fst-italic" style="color: var(--colortext3);" @click.prevent="handleTabClick('reviews')">Đánh giá ({{ totalRatings }})</a>
                            </div>
                            ...
                            <div class="d-flex align-items-center gap-2">
                                <div class="">
                                    ...
                                    <input class="form-control form-date" type="date" v-model="selectedDate" id="form-date" :min="selectedDate">
                                </div>
                                <div>
                                    <label class="bk-detail__label" for="form-timestart">Giờ bắt đầu</label>
                                    <input type="time" class="form-control form-date" name="" id="" placeholder="Giờ bắt đầu" value="06:00:00" disabled>
                                </div>
                                <div>
                                    <label class="bk-detail__label" for="form-timestart">Giờ kết thúc</label>
                                    <input type="time" class="form-control form-date" name="" id="" placeholder="Giờ bắt đầu" value="08:00:00" disabled>
                                </div>
                            </div>
                            ...
                            <button class="d-block btn-find" style="width: 32%; padding: 15px 0 17px 0;" @click="openBookingForm">Đặt ngay</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <!-- Form đặt sân -->
        <div class="booking-form-modal" v-if="showBookingForm">
            <div class="booking-form-container">
                ...
                <div v-if="!bookingSuccess" class="booking-form-content">
                    <div class="booking-form-info">
                        <div class="booking-info-card">
                            ...
                            <div class="info-content">
                                ...
                                <div class="info-item" v-if="selectedSlot">
                                ...
                                <div class="price-tag" v-if="selectedSlot">
                    ...
                    <div class="alert custom-alert-danger" v-if="bookingError">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control custom-input" v-model="customerName" placeholder="Nhập họ tên của bạn">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="tel" class="form-control custom-input" v-model="customerPhone" placeholder="Nhập số điện thoại">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="email" class="form-control custom-input" v-model="customerEmail" placeholder="Nhập email của bạn">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control custom-textarea" v-model="bookingNote" rows="3" placeholder="Thêm ghi chú (nếu có)"></textarea>
                    </div>
                    ...
                    <div class="form-group payment-section">
                        ...
                        <div class="payment-options">
                            <div class="payment-option" :class="{ 'active': paymentMethod == 1 }" @click="paymentMethod = '1'">
                                <input class="form-check-input" type="radio" v-model="paymentMethod" name="payment" id="payment1" value="1">
                            ...
                            <div class="payment-option" :class="{ 'active': paymentMethod == 2 }" @click="paymentMethod = '2'">
                                <input class="form-check-input" type="radio" v-model="paymentMethod" name="payment" id="payment2" value="2">
                                <div class="payment-icon">
                        ...
                        <div v-if="paymentMethod == 2" class="bank-transfer-info">
                            ...
                            <div class="bank-details">
                                ...
                                <div class="bank-row">
                                    ...
                                    <button class="copy-btn" @click="copyToClipboard('1234567890')" title="Sao chép số tài khoản">
                                ...
                                <div class="bank-row">
                                    ...
                                    <span class="bank-value message-value">{{ yard_store?.Ten_san }} - {{ selectedDate }} - {{ customerName }}</span>
                                    <button class="copy-btn" @click="copyToClipboard(`${yard_store?.Ten_san} - ${selectedDate} - ${customerName}`)" title="Sao chép nội dung chuyển khoản">
                                        <i class="bi bi-clipboard"></i>
                                    </button>
                                </div>
                    ...
                    <div class="form-actions">
                        ...
                        <button class="confirm-btn" @click="submitBooking" :disabled="isProcessing">
                ...
                <div v-else class="booking-success-content">
                    ...
                    <div class="success-content">
                        ...
                        <p class="contact-message">Chúng tôi sẽ liên hệ với bạn qua số điện thoại để xác nhận đơn đặt sân trong thời gian sớm nhất.</p>
                    </div>
                    ...
                    <div class="booking-summary">
                        ...
                        <div class="summary-item" v-if="selectedSlot">
                            ...
                            <div class="summary-content">
                                ...
                                <div class="summary-value">{{ formatTime(selectedSlot.Gio_bat_dau) }} - {{ formatTime(selectedSlot.Gio_ket_thuc) }}</div>
                            </div>
                        </div>
                        <div class="summary-item price-item" v-if="selectedSlot">
                            <div class="summary-icon">
                                <i class="bi bi-cash-coin"></i>
                            </div>
                            <div class="summary-content">
                        ...
                        <div class="bkdt__box-rating m-lg-5 m-2 me-lg-0 p-5">
                                    ...
                                    <div class="d-flex align-items-center gap-2 mb-1">
                                        <template v-for="star in 5" :key="star">
                                            <i class="bi bi-star-fill fs-4" 
                                               :class="star <= averageRating ? 'color-star' : 'color-star-gray'"></i>
                                        </template>
                                    </div>
                            ...
                            <div class="d-flex align-items-center justify-content-between my-1">
                                ...
                                <div class="bkdt__percent-rating">
                                    <div class="bkdt__percent-rating-child" 
                            ...
                            <div class="d-flex align-items-center justify-content-between my-1">
                                ...
                                <div class="bkdt__percent-rating">
                                    <div class="bkdt__percent-rating-child" 
                                         :style="{ width: getRatingPercentage(0) + '%' }"></div>
                                </div>
                                <p class="fs-5 fst-italic m-0 text-end" style="color: var(--colortext3); width: 76px;">
                                                    ...
                                                    <div class="d-flex align-items-center gap-1 mb-1">
                                                        <template v-for="star in 5" :key="star">
                                                            <i class="bi bi-star-fill fs-5" 
                                                               :class="star <= review.So_sao ? 'color-star' : 'color-star-gray'"></i>
                                                        </template>
                                                    </div>
                        ...
                        <div v-for="yard in popular_yards.slice(0, 4)" :key="yard.id" class="col-12 col-lg-3 col-md-6 p-0">
                            <div class="yard">
                                <a :href="`/san/${yard.id}`" class="link-img-p">
                                    <img :src="getImageSrc(yard.Hinh_anh)" :alt="yard.Ten_san" style="width: 100%;">
                                    </a>
